# Stack Research: Platform Capability Surface

**Domain:** Conversational AI agent platform capabilities (Google CX Agent Studio / Gemini Enterprise for Customer Experience) for an insurance FNOL claims demo specification
**Researched:** 2026-07-08
**Confidence:** MEDIUM-HIGH (core architecture, tools, deployment verified against current official docs; a few marketing-only claims flagged LOW/MEDIUM pending hands-on confirmation)

> **Reading note for the implementation team:** This is not a code stack — it is the capability surface of the target platform. Every row is tagged **[BUILT-IN]** (little/no effort to demo), **[CUSTOM TOOL]** (requires a Python/OpenAPI tool the implementation team writes), or **[MOCK DATA]** (requires seeded/simulated content). Product is very new — Gemini Enterprise for CX was announced 2026-01-11 at NRF, CX Agent Studio reached **General Availability on 2026-02-04**. Docs are evolving monthly; re-verify anything marked MEDIUM/LOW before the build starts.
...
**Implication for the spec:** Describe V2 as one root agent ("Alex" or successor) with purpose-built sub-agents — e.g., Intake, Damage-Assessment (multimodal), Decision/Triage, Escalation-Handoff, Cross-Sell — each with its own natural-language instructions, examples, and tools. This maps directly onto CX Agent Studio's documented architecture and is exactly the pattern its own sample app (Cymbal Retail) demonstrates.
...
**Best-practice guidance directly from Google's own docs** (applies to the spec): wrap external/mock APIs in Python tools rather than exposing raw OpenAPI schemas with excess fields (reduces token usage/hallucination); avoid instructing the agent to chain multiple tool calls itself — have one tool internally orchestrate dependent mock-backend calls for determinism ("the internals of a tool are fully deterministic, but a tool call orchestrated by an agent is not").
...
**For the dollar-threshold auto-approve vs. human-assessor branch:**
...
**For voice + mid-demo language switching:**
...
**For the damage-photo multimodal moment:**
...
**For the backend claims-processing reveal (transcript → summary → drafted email):**
...
---
*Stack research for: Google CX Agent Studio / Gemini Enterprise for Customer Experience platform capabilities*
*Researched: 2026-07-08*
