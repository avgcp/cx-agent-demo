...
**Success Criteria** (what must be TRUE of the deployed agent):
  1. A customer can attach a photo of the damage, and the agent reports what it can actually see in that image — confirming or contradicting what the customer reported — while the **claim amount continues to come from the deterministic tariff and never from the vision read**. (Confirm path verified live 2026-08-09, quick task 260809-nt7: driven end to end through the real widget embed — a real cracked-MacBook photo uploaded in the browser, read correctly, priced from the tariff at $840, auto-approved as `CLM-24442` — the first time exercised anywhere other than the API and the simulator.)
  2. The anti-fraud path holds: a reported crack that is absent from the photo sets `photo_contradiction`, routes to human review under DL-5, and is communicated to the customer without accusing them of anything. An unusable photo gets exactly one retry before a human is involved. **Accepted limitation (2026-08-06):** the photo confirms whether the reported *damage* is visible; it does **not verify that the photographed device is the insured device**. Wrong-object detection was attempted twice — chat v4 `7710088b` (model names the device category) and chat v5 `5ccd34a4` (model reports physical features, tool infers) — and failed live both times, because the model already knows what the policy covers and conforms its identity report to that rather than to the image. It was therefore deliberately removed in chat v6 `56a8b22a` rather than left in place reading as protection it does not provide. Demos must use matching device/policy pairs.
  ...
  4. A `WEB_UI` deployment with file upload runs alongside the existing GTP phone deployment, and **the phone deployment continues to serve its pinned version unchanged**. (The `WEB_UI` deployment's file upload is now proven through the real browser embed and required no Cloud Storage configuration; the phone deployment remains pinned and untouched at v11 `b17c9a26`.)
...
**Plans:** 1 complete, 1 in progress, 2 outstanding
...
- [~] 05-02 — Decision card + quick actions — built, deployed and **rendering-verified live** through the real v1.16 widget embed on chat v7 `bb14cdcc` (2026-08-09, `CLM-24442`, `$840`/`$815` auto-approved). What keeps it `[~]` is `cover_offer_actions` — never fired, defective against the SDK's `quick_actions` contract — plus the decision turn producing no spoken sentence alongside the card. Wrong-subject photo guard **resolved by removal** (quick task 260806-u21, chat v6 `56a8b22a`): proven unfixable by prompting after two attempts, so the guard was removed and the limitation documented rather than shipped as false protection. See 05-02-SUMMARY.md
