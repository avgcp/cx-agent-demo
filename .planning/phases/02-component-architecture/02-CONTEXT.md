...
### Repair-Partner Lookup (TOOL-01, TOOL-02) — REVERSED post-plan-checker (2026-07-20)
- **D-16:** **Repair-partner recommendation is OUT OF SCOPE.** The autonomous-path backend-reveal email stays exactly as Phase 1 already authored it (Beat 9: "confirming the approval, claim number, and next steps" — no partner named). **No `repair_partner_lookup` tool** appears in the §6 inventory, and §5's Backend Claims-Processing agent does not reference a repair partner.
  - **Why reversed:** the original D-16 (keep it as a mock Python tool) rested on a false premise — that Phase 1's email already named a "recommended repair partner." The plan-checker grep-verified it does not, and that Section 4 (the single source of truth for mock data) contains **zero** repair-partner seed records. Adding the tool would have forced either invented mid-phase data (breaking DATA-04's "every figure traces to a named §4 field") or a dangling §4 citation that fails when Phase 3 cross-references it. Phase 1's actual email already matches the "out of scope" baseline.
  - **What still holds:** **the demo has ZERO RAG / Data Store components — every tool in the inventory is a deterministic Python tool** (this is now even cleaner: one fewer tool, no new seed data, no edits to completed Phase 1 sections). This still supersedes the research's `repair_partner_search` Vertex AI Search Data Store (`research/ARCHITECTURE.md` §Recommended Composition) and honors SC#4. The §6 inventory notes the data-store row count as **0 (all structured mock data lives inside Python tools)**, with a one-line "path to production" footnote that a real deployment could add a partner directory (RAG or Python) later.
...
- **Full tool roster completeness:** the inventory must include not just the backend lookups named in SC#4 (policy, coverage, claim creation, device-upsell/coverage-gap check, damage valuation) but also the supporting tools the architecture needs — e.g. `evaluate_claim` (deterministic threshold/flag math), `claim_number_generator`, `draft_email` (dual-template for the two Backend output modes), and the `end_session` **system tool** for escalation/close. **No `repair_partner_lookup`** (out of scope per reversed D-16). Inventory completeness is correctness, not a new decision.
...
- **Repair-partner recommendation** — dropped from scope entirely (reversed D-16); no seed data exists and Phase 1's email already omits it. Retained only as a one-line "path to production" footnote in §6 (a real deployment could add a partner directory later), not a build item.
